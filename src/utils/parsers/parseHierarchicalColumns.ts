...
export function parseHierarchicalColumns(
  ...
  for (let r = dataStartRow; r < sheet.rows.length; r++) {
    ...
    for (let d = 0; d <= leafDepthIdx; d++) {
      ...
      // Dedup ALL nodes (parents and leaves) by (depth + path). First row at a
      // given (leafDepth, path) wins and owns its attributes; subsequent rows
      // collapse. Note: parentByKey now holds leaves too despite its name.
